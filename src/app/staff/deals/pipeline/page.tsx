...
export default async function StaffDealsPipelinePage() {
  const profile = await getCurrentProfile();
  const currentUserId = profile?.user_id || '00000000-0000-0000-0000-000000000002';

  const pipelineData = await getPipelineDeals('ASSIGNED', currentUserId);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-900">My Sales Pipeline Board</h1>
          <p className="text-xs text-slate-500">
            Visual stage progression board for your assigned sales opportunities.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Link href="/staff/deals">
      ...
      {/* Filter toolbar */}
      <DealFilters basePath="/staff/deals" showPipelineToggle={true} currentView="pipeline" />
