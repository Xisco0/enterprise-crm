...
export default async function AdminEditLeadPage({ params }: PageProps) {
  const { id } = await params;
  const [lead, staffList] = await Promise.all([
    getLeadById(id),
    getAllStaffProfiles(),
  ]);
    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      ...
      <div>
        ...
        <p className="text-xs text-slate-500">
          Modify contact information, update pipeline qualification, or reassign account representative.
        </p>
      </div>
