...
const navGroups: NavGroup[] = [
  {
    groupName: 'Personal Workspace',
    items: [
      { label: 'My Workspace', href: '/staff/dashboard', icon: LayoutDashboard },
      { label: 'My Performance', href: '/staff/reports', icon: BarChart3 },
    ],
  },
  {
    groupName: 'Assigned Pipeline',
    items: [
      { label: 'My Leads', href: '/staff/leads', icon: UserCheck },
      { label: 'My Client Accounts', href: '/staff/customers', icon: Building2 },
      { label: 'My Deals & Pipeline', href: '/staff/deals', icon: TrendingUp },
      { label: 'My Activity Hub', href: '/staff/interactions', icon: MessageSquare },
      { label: 'My Agenda & Tasks', href: '/staff/tasks', icon: CheckSquare },
    ],
  },
];
      ...
      <div>
        <div className="flex h-16 items-center justify-between border-b border-slate-200 px-5">
          <Link href="/staff/dashboard" className="flex items-center gap-3 group">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-blue-600 text-white shadow-xs font-heading font-bold text-sm tracking-tight transition-transform group-hover:scale-105">
              AP
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                ...
                <span className="rounded bg-blue-50 px-1.5 py-0.5 text-[9px] font-sans font-semibold text-blue-700 border border-blue-200">
                  v2.0
                </span>
              </div>
              <span className="text-[10px] text-slate-500 font-sans flex items-center gap-1 mt-0.5 font-medium">
                <Briefcase className="w-3 h-3 text-blue-600" /> Sales Representative
              </span>
            </div>
          </Link>
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-50/80 p-3 text-[11px] space-y-1">
          <div className="flex items-center justify-between">
            <span className="flex items-center gap-1.5 font-semibold text-slate-800">
              ...
              Scoped Privacy Enforced
            </span>
            <span className="text-[10px] font-sans font-medium text-slate-500">Staff Mode</span>
          </div>
          <p className="text-[10px] text-slate-500 leading-tight">
            Assigned records and personal activity only.
          </p>
        </div>
      </div>
    </div>
